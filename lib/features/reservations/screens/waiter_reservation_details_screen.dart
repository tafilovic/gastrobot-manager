...
///
/// Extracted from `_buildWaiterDetails` in `ReservationDetailsScreen`.
...
class WaiterReservationDetailsScreen extends StatelessWidget {
  ...
  Future<void> _openAcceptSheet(BuildContext context) async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => AcceptReservationSheet(order: order),
    );
  }
    ...
    Future<void> submit(StateSetter setSheetState) async {
      ...
      if (ok) {
        ...
        setSheetState(() {
          submitting = false;
          fieldError =
              provider.rejectError ?? l10n.rejectErrorFallback;
        });
      }
    }
