...
import 'package:gastrobotmanager/features/zones/widgets/zone_order_bill_sheet.dart';
import 'package:gastrobotmanager/l10n/generated/app_localizations.dart';

/// New order screen for table ordering. Shows categories and menu items with add-to-cart.
/// [orderForZone] is the venue table this order is for (bill and API use [ZoneModel.id]).
class ZoneOrderScreen extends StatefulWidget {
  const ZoneOrderScreen({super.key, required this.orderForZone});

  final ZoneModel orderForZone;

  @override
  State<ZoneOrderScreen> createState() => _ZoneOrderScreenState();
}

class _ZoneOrderScreenState extends State<ZoneOrderScreen> {
  ...
  void _load() {
    ...
    if (venueId != null) {
      context.read<ZoneOrderMenuProvider>().load(venueId);
    }
  }
  ...
  void _onCartPressed() {
    ...
    showZoneOrderBillSheet(
      context: context,
      cartListenable: _cartNotifier,
      initialCart: Map.from(_cart),
      menuProvider: context.read<ZoneOrderMenuProvider>(),
      ...
      orderForZone: widget.orderForZone,
    );
  }
  ...
  Future<void> _submitOrder(String tableId) async {
    ...
    final menuProvider = context.read<ZoneOrderMenuProvider>();
    ...
    try {
      await context.read<ZonesApi>().createVenueOrder(
        venueId,
        CreateZoneOrderRequest(
          tableId: tableId,
          userId: userId,
          orderItems: lines,
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final provider = context.watch<ZoneOrderMenuProvider>();
