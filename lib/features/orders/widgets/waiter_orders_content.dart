  ...
  List<PendingOrder> _applyHistoryFilters(List<PendingOrder> orders) {
    ...
    return orders.where((order) {
      ...
      if (f.orderContentTypes.isNotEmpty) {
        if (f.orderContentTypes.contains(
              HistoryOrderFilters.orderContentFood,
            ) &&
            !order.items.any((i) => i.type == 'food')) {
          return false;
        }
        if (f.orderContentTypes.contains(
              HistoryOrderFilters.orderContentDrink,
            ) &&
            !order.items.any((i) => i.type == 'drink')) {
          return false;
        }
      }
      return true;
